...
function saveFiles(){
  ...
  if ( isset($_SESSION["user"]) &&
    in_array($_SESSION["user"], $allowed_emails)) {
        ...
        createOrUpdateDir($folder."/cruzada");
        // cria o diretório '/yyy_mm_dd/parceiro'
        createOrUpdateDir($folder."/parceiro");
        // cria o diretório '/yyy_mm_dd/parceiro/cruzada'
        createOrUpdateDir($folder."/parceiro/cruzada");

        // copia os dois modelos de cruzada
        // para o diretorio da data ( yyy_mm_dd )
				recursive_copy("modelo_estadao/cruzada", $folder."/cruzada");
        recursive_copy("modelo_parceiro/cruzada", $folder."/parceiro/cruzada");

        // sobrescreve o arquivo de dados
        // com o novo texto
        if( file_exists($folder."/cruzada/dados") ){
          /* create a stream context telling PHP to overwrite the file */
          $options = array('ftp' => array('overwrite' => true));
          $stream = stream_context_create($options);

          if( !file_put_contents($folder."/cruzada/dados/".$fname, $text, 0, $stream) ){
        ...
        if( file_exists($folder."/parceiro/cruzada/dados") ){
          /* create a stream context telling PHP to overwrite the file */
          $options = array('ftp' => array('overwrite' => true));
          $stream = stream_context_create($options);

          file_put_contents($folder."/parceiro/cruzada/dados/".$fname, $text, 0, $stream) or die("Não foi possível salvar o arquivo de texto (parceiro)");
				}else{
        ...
        // se foi enviado o arquivo de imagem...
          ...
          // define o caminho onde será salva a imagem
	        $uploadfile = $folder."/cruzada/images/" . basename($imgname);
          $partnerfile = $folder."/parceiro/cruzada/images/" . basename($imgname);

          // salva na pasta do modelo do estadão
          if( file_exists( $folder."/cruzada/images" ) ){

						if( move_uploaded_file($img['tmp_name'], $uploadfile) ){
              // copia a imagem para o modelo dos parceiros
              // Em seguida cria o zip do parceiro
              if( file_exists( $folder."/parceiro/cruzada/images" ) ){
                recursive_copy( $uploadfile, $partnerfile );

                if( createZip( $_POST['pDir'], $folder."/parceiro/cruzada/" ) ){
                  echo json_encode("../".$_POST['pDir']."/cruzada/ ");
                  exit;
                }else{
...
function createZip($date, $copiedFolder){
	$success = false;
	// Get real path for our folder
	$rootPath = realpath($copiedFolder);

	// Initialize archive object
	$zip = new ZipArchive();
	$zip->open( '../../cruzada_'.$date.'.zip', ZipArchive::CREATE | ZipArchive::OVERWRITE );
